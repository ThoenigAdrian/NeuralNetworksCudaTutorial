...
__global__ void feed_forward_and_backprop(float *z_matrix, float* activation_matrix, float* weight_matrix, float* biases, int* shape, int shape_length, float* outputs, float* error, float* error_weights,
	int* activation_offset_table, int* bias_offset_table, int* z_offset_table, int* wm_offset_table, float* error_summed)
{
	...
	// This is not an efficient sum reduction but for now im just using one thread per neuron will change in later itartions
	for (int layer = 0; layer < shape_length - 1; layer++)
	{
		// for each weight one thread
		if (threadIdx.y < shape[layer+1] && threadIdx.x == 0)
		{
			// I know this is yet not efficient for many inputs . Problems is it's not that easy because we need to access the same memory address when summing
			// There is an summing algorithm which is more efficient but maybe later
			for (int input_nr = 0; input_nr < blockDim.x; input_nr++)
			{
				error_summed[bias_offset_table[layer] + threadIdx.y] += error[error_offset_table[layer] + input_nr * (shape[layer+1])+ threadIdx.y];
			}
...
int main()
{
	...
	const size_t bytes_error_summed = bytes_biases;
	...
	float *host_error_summed = (float*)malloc(bytes_error_summed);
	memset(host_error_summed, 0, bytes_error);

	float *host_error_weights = (float*)malloc(bytes_error_weights);
	memset(host_error_weights, 0, bytes_error_weights);

	// Allocate GPU device  memory
	float *d_biases, *d_weights, *d_activations, *d_z, *d_error, *d_error_weights, *d_output, *d_error_summed;
	...
	cudaMalloc(&d_error_summed, bytes_error_summed);
	...
	cudaMemcpy(d_error_summed, host_error_summed, bytes_error_summed, cudaMemcpyHostToDevice);
	...
	feed_forward_and_backprop << <dimGrid, dimBlock >> > (d_z, d_activations, d_weights, d_biases, d_shape, NUM_LAYERS_TOTAL, d_output, d_error, d_error_weights, d_activation_offset_table, d_bias_offset_table, d_z_offset_table, d_weight_matrix_offset_table, d_error_summed);
	...
	cudaMemcpy(host_error_summed, d_error_summed, bytes_error_summed, cudaMemcpyDeviceToHost);
	...
	print_error_bias_matrix_numpy_style(host_error_summed, SHAPE, NUM_LAYERS_TOTAL);
